...
def download_file(filename, directory):
    try:
        print(f"Sending file from: {directory}, filename: {filename}", flush=True)  # Debug statement
...
def home():
    ...
    if (watermark_logo_url==None):
        user_folder, input_image_url, watermark_logo_url = create_user_folder(current_user.get_preference().user_id)
...
def create_user_folder(user_id):
    ...
    return user_folder_path, input_image_url, watermark_logo_url
